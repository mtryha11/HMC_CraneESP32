...
namespace {  // Helpers
    ...
    bool spurt(const String& fn, const String& content) {
        ...
        bool w=NVS.setString(fn,content);
...
void WiFiSettingsClass::portal() {
    ...
    if (secure && password.length()) {
        ...
        // WiFi.softAP(hostname.c_str(), "geyes2022!");
    } else {
    ...
    http.on("/", HTTP_GET, [this, &http, &num_networks, &redirect]() {
        ...
        if (slurp("/wifi-pass").length()) http.sendContent("##**##**##**");
    ...
    http.on("/", HTTP_POST, [this, &http]() {
        ...
        if (WiFiSettingsLanguage::multiple()) {
            if (! spurt("/WiFi-lang", http.arg("language"))) ok = false;
        ...
        if (pw != "##**##**##**") {
            if (! spurt("/wifi-pass", pw)) ok = false;
        }
...
void WiFiSettingsClass::begin() {
    ...
    String user_language = slurp("/WiFi-lang");
    user_language.trim();
